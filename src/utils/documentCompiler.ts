...
const colorTagCompiler = (contents: string) => {
  return contents
    ...
    .replace(
      /<무지개>([\s\S]*?)<\/무지개>/gi,
      `<span class="${styles.rainbow}">$1</span>`
    );
};
...
const utilityTagCompiler = (contents: string) => {
  return contents
    ...
    .replace(
      /<항목>([\s\S]*?)<\/항목>/gi,
      "<li style='list-style: disc';>$1</li>"
    )
    .replace(
      /<어록>([\s\S]*?)<\/어록>/gi,
      `<div class="${styles.quote}";>$1</div>`
    )
    .replace(
      /<블록>([\s\S]*?)<\/블록>/gi,
      `<div class="${styles.block}";>$1</div>`
    )
    .replace(/<강조>([\s\S]*?)<\/강조>/gi, "<strong>$1</strong>")
    .replace(
      /<취소선>([\s\S]*?)<\/취소선>/gi,
      `<del style="color: ${theme.gray}">$1</del>`
    )
    .replace(
      /<빙글빙글>([\s\S]*?)<\/빙글빙글>/gi,
      `<div class="${styles.spin}">$1</div>`
    )
    .replace(
      /<삐슝빠슝>([\s\S]*?)<\/삐슝빠슝>/gi,
      `<div class="${styles.shake}">$1</div>`
    )
    .replace(
      /<소제목>([\s\S]*?)<\/소제목>/gi,
